...
class Gait:
    ...
    _foot_trajectories: npt.NDArray[np.float64]  # (Leg, Knot/Point, Coordinates) | Relative Coordinates
    _time_anchors:      npt.NDArray[np.float64]  # (Leg, time anchor)
    _time_reference:    float = 0  # Calculated last time step
    ...
    def calculate_foot_trajectories(self, node_count:int) -> None:
        ...
        self._foot_trajectories = np.empty((LEG_COUNT, node_count, 3), np.float64)  # (Leg, Control Point, Coordinates) | Relative Coordinates
        self._time_anchors      = np.empty((LEG_COUNT, node_count), np.float64)     # (Leg, time anchor)

        for i in range(LEG_COUNT):
            control_points = self.leg_control_points[i]
            self._foot_trajectories[i], self._time_anchors[i] = interpolator.compute_interpolated_points(control_points, node_count)
    
    # TODO: Optimise
    def _calculate_time_reference(self) -> None:
        for anchor, phase_offset in zip(self._time_anchors[:LEG_COUNT], self.leg_phase_offset[:LEG_COUNT]):
            calculated_time = anchor * (phase_offset + 1)
            
            if calculated_time > self._time_reference:
                self._time_reference = calculated_time
